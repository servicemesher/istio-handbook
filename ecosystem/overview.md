...
2018 年 6 月 26 日，HashiCorp 发布了 [HashiCorp Consul 1.2](https://www.consul.io/)。这个版本主要新增了一个新的功能叫做 Connect, 它能够将现有的 Consul 集群自动转变为 Service Mesh 的解决方案。
...
2018 年 7 月，蚂蚁金服开源了云原生网络代理 [MOSN](https://github.com/mosn/mosn)。MOSN 是一个使用 Go 语言开发的数据平面代理，为服务提供多协议、模块化、智能化、安全的代理能力。MOSN 可以与任何支持 xDS API 的 Service Mesh 集成，亦可以作为独立的四、七层负载均衡，API Gateway、云原生 Ingress 等使用。

MOSN 项目已于 2019 年 8 月加入 CNCF Landscape，由于有着蚂蚁金服的实力加成，目前该项目社区比较活跃。

### Kong kuma

2019年9月10日，Kong正式宣布开源旗下 Service Mesh 项目 [Kuma](https://github.com/Kong/kuma)。Kuma 定位是一个通用的服务网格，不同于市场上的大多数服务网格项目，它的设计初衷是在 Kubernetes 生态系统内部和外部都能工作，这包括虚拟机、容器、遗留环境以及 Kubernetes。除此之外，Kuma 引入 Envoy 作为数据平面代理，并引入自家的明星产品 Kong 作为 API 网关，该网关管理组织与部署现代微服务的各种方法之间的信息流。
...
[AWS APP Mesh](https://aws.amazon.com/app-mesh/) 是 AWS 在 re:Invent 2018 大会上发布的一款新服务，并在 2019 年 4 月发布了正式版。App Mesh 作为 AWS 原生服务网格，与 AWS 现有产品簇完全集成，包括：
...
Google 作为 Istio 的主要发起方之一，在 Service Mesh 方面投入了大量资源。在 Google Cloud 上就有 GKE mesh、[Google Cloud Service mesh](https://cloud.google.com/anthos/service-mesh) 和 Google Traffic Director 等多种产品。
...
微软早在 2018 年 10 月发布 [Service Fabric Mesh](https://docs.microsoft.com/en-us/azure/service-fabric-mesh/service-fabric-mesh-overview) 预览版，但时至今日，仍然没有看到正式版本的推出，相关资料也比较缺乏。反倒是在 2019 年 5 月，微软在 KubeConf 上高调推出 SMI（Service Mesh Interface）。
...
Red Hat 于 2018 年推出了 [OpenShift Service Mesh](https://www.openshift.com/learn/topics/service-mesh) 技术预览版，正是基于 Istio 实现，可以为 OCP 客户提供在其 OpenShift 集群上部署和使用 Istio 的能力。此外，Red Hat 还创建了一个名为 Maistra 的社区项目，为 Istio 集成了诸多开源项目，以降低 Istio 的使用成本。Red Hat 凭借在私有化交付市场中的高占有率，实施落地了众多 Service Mesh 案例。

### Aspen Mesh

[Aspen Mesh](https://aspenmesh.io/) 来自大名鼎鼎的 F5 Networks 公司，基于 Istio 构建，定位企业级服务网格，口号是”Service Mesh Made Easy”。
...
- 蚂蚁金服：蚂蚁金服将 Service Mesh 作为其云上产品[金融分布式架构 SOFAStack](https://help.aliyun.com/document_detail/147610.html) 中的一部分，基于  Istio 构建的 Service Mesh 的控制平面，并积极贡献 Istio 社区，基于 MOSN 提供数据平面服务，在兼容 xDS 标准的基础上，对控制平面进行了部分优化和增强。
...
- [Service Mesh2018年度总结](https://skyao.io/publication/201902-service-mesh-2018-summary/)
